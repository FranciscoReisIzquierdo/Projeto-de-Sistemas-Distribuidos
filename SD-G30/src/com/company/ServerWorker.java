...
public class ServerWorker implements Runnable {
    ...
    public ServerWorker(Socket socket, BaseDados dados) throws IOException {
        this.socket = socket;
        this.dados= dados;
    ...
    public void run() {
        try {
            while (!this.socket.isClosed()) {
                ...
                switch(opcao){
                    case 0: verificaAutenticacaoCliente();
                        break;

                    case 1: registarCliente();
                        break;
                    case 2: verificaAutenticacaoAdmin();
                        break;
                    case 3: reservarVoo();
                        break;
                    case 4: encerramentoDia();
                        break;
                    case 5: inserirVoo();
                        break;
                    case 6: listarVoos();
        ...
        Servidor.threads.decrementAndGet();
    }
    ...
    private void registarCliente() throws IOException {
        ...
        this.dados.registosLock.lock();
        if(dados.getClientes().containsKey(username)){
            this.dados.registosLock.unlock();
    ...
    private void encerramentoDia() throws IOException {
        ...
        this.dados.voosLock.lock();
        if(this.dados.getCancelados().contains(data)) {
            this.dados.voosLock.unlock();
            output.writeBoolean(false);
            output.flush();
        } else {
    ...
    private void inserirVoo() throws IOException {
        ...
        Pair entry = new Pair(origem, destino);
        if(this.dados.getVoo(entry)!= null){
            output.writeBoolean(false);
            output.flush();
        }
        else{
    ...
    public void cancelarVoo() throws IOException {
        ...
        this.dados.canceladosLock.lock();

        if(!dados.getCancelados().contains(reserva.getDia())) {
            this.output.writeBoolean(true);
            this.dados.voosLock.lock();
            ...
            this.dados.canceladosLock.unlock();
            System.out.println("Reserva " + reserva.getCodigo() + " cancelada pelo cliente: " + this.username);
        } else {
            this.dados.canceladosLock.unlock();
            this.output.writeBoolean(false);
            System.out.println("A reserva não pode ser cancelada porque o dia já foi encerrado");
        }
        this.output.flush();
    }
    ...
    private void reservarVoo() throws IOException {
        List<Pair> voos = new ArrayList<>(this.dados.getListaVoos().keySet());
        ...
        for(int i = 0; i < opcao.size() - 1; i++){
            Pair entry =new Pair(opcao.get(i), opcao.get(i + 1));
            Voo voo = this.dados.getVoo(entry);
    ...
    private void encontraDestinos(String origem, Set<String> visitados, Set<String> destinos, List<Pair> voos){
    ...
    private void dfs(List<Pair> voos, String origem, String d, List<String> visitados, List<String> caminho, List<List<String>> caminhos){
        caminho.add(origem);
    ...
    private List<String> adjacentes(String origem, List<Pair> voos){
